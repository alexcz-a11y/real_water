...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (manifest.declarations.length !== 86) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 7) throw new Error("Packed Prewarm Manifest version must be 7.");',
    'if (manifest.version !== 7) throw new Error("Packed manifest version must be 7.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 33) throw new Error("Packed diagnostics capture names drifted.");',
