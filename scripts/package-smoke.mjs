...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (profile.reflection.ssr.roughnessCutoff !== 0.5 || profile.reflection.ssr.blurQuality !== 2 || profile.reflection.ssr.mipCount !== 5 || profile.reflection.ssr.history.mode !== "temporal-reproject-specular" || profile.reflection.ssr.history.accumulate !== true || profile.reflection.ssr.history.maxFrames !== 32) throw new Error("Packed Quality Profile SSR policy drifted.");',
    ...
    'if (manifest.declarations.length !== 55) throw new Error("Packed Prewarm Manifest work count drifted.");',
