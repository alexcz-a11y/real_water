...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (manifest.declarations.length !== 62) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 5) throw new Error("Packed Prewarm Manifest version must be 5.");',
    'if (manifest.version !== 5) throw new Error("Packed manifest version must be 5.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 23) throw new Error("Packed diagnostics capture names drifted.");',
