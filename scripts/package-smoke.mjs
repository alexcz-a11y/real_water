...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (profile.interaction.anchorCount !== 1 || profile.interaction.field.radiusMetres !== 48 || profile.interaction.field.edgeFadeMetres !== 8 || profile.interaction.field.maxActiveDisturbances !== 128 || profile.interaction.field.snapshotBanks !== 2 || profile.interaction.field.maxSnapshotAgeTicks !== 1 || profile.interaction.field.radialImpactRoute !== "analytic-uniform-array" || profile.interaction.field.directionalWakeRoute !== "analytic-uniform-array") throw new Error("Packed Quality Profile interaction policy drifted.");',
    ...
    'if (profile.whitecaps.mode !== "unified-source-ping-pong" || profile.whitecaps.sourceLayout !== "whitecap-wake-impact-combined" || profile.whitecaps.localHistoryBanks !== 2 || profile.whitecaps.maxLocalSources !== 128 || profile.whitecaps.foamTimelineCapacityTicks !== 128) throw new Error("Packed Quality Profile unified foam policy drifted.");',
    ...
    'if (manifest.declarations.length !== 122) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 10) throw new Error("Packed Prewarm Manifest version must be 10.");',
    'if (manifest.version !== 10) throw new Error("Packed manifest version must be 10.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 40) throw new Error("Packed diagnostics capture names drifted.");',
