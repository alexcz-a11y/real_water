...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (profile.postTraaComposition.mode !== "ordered-declarative-stages" || profile.postTraaComposition.stages[0]?.id !== "secondary-particles" || profile.postTraaComposition.stages[0]?.after !== "traa") throw new Error("Packed post-TRAA composition plan drifted.");',
    ...
    'if (manifest.declarations.length !== 103) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 9) throw new Error("Packed Prewarm Manifest version must be 9.");',
    'if (manifest.version !== 9) throw new Error("Packed manifest version must be 9.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 36) throw new Error("Packed diagnostics capture names drifted.");',
