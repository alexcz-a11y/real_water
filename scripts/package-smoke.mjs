...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (manifest.declarations.length !== 79) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 6) throw new Error("Packed Prewarm Manifest version must be 6.");',
    'if (manifest.version !== 6) throw new Error("Packed manifest version must be 6.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 29) throw new Error("Packed diagnostics capture names drifted.");',
