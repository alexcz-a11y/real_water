...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (profile.whitecaps.mode !== "unified-source-ping-pong" || profile.whitecaps.sourceLayout !== "whitecap-wake-impact-combined" || profile.whitecaps.localHistoryBanks !== 2 || profile.whitecaps.maxLocalSources !== 128 || profile.whitecaps.sourceTimelineCapacityTicks !== 128) throw new Error("Packed Quality Profile unified foam policy drifted.");',
    "const manifest = realWater.createMinimalWaterPrewarmManifest(profile, { width: 320, height: 180 });",
    'if (manifest.declarations.length !== 87) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 30) throw new Error("Packed diagnostics capture names drifted.");',
