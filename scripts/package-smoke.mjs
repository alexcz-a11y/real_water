...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (profile.postTraaComposition.mode !== "ordered-declarative-stages" || profile.postTraaComposition.stages.length !== 2 || profile.postTraaComposition.stages[0]?.id !== "secondary-particles" || profile.postTraaComposition.stages[0]?.after !== "traa" || profile.postTraaComposition.stages[1]?.id !== "lens-wetness" || profile.postTraaComposition.stages[1]?.after !== "secondary-particles" || profile.postTraaComposition.lensWetness.mode !== "bounded-emergence-decay" || profile.postTraaComposition.lensWetness.trigger !== "waterline-emergence-impulse") throw new Error("Packed post-TRAA composition plan drifted.");',
    ...
    'if (manifest.declarations.length !== 129) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 11) throw new Error("Packed Prewarm Manifest version must be 11.");',
    'if (manifest.version !== 11) throw new Error("Packed manifest version must be 11.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 41) throw new Error("Packed diagnostics capture names drifted.");',
