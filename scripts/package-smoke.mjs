...
try {
  ...
  const runtimeSmoke = [
    ...
    'if (manifest.declarations.length !== 94) throw new Error("Packed Prewarm Manifest work count drifted.");',
    ...
    'if (realWater.PREWARM_MANIFEST_VERSION !== 8) throw new Error("Packed Prewarm Manifest version must be 8.");',
    'if (manifest.version !== 8) throw new Error("Packed manifest version must be 8.");',
    ...
    'if (!Array.isArray(diagnostics.DIAGNOSTICS_CAPTURE_NAMES) || diagnostics.DIAGNOSTICS_CAPTURE_NAMES.length !== 34) throw new Error("Packed diagnostics capture names drifted.");',
