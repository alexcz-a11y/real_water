...
describe("createThreeHostLifecycleAdapter", () => {
  ...
  it("prewarms and disposes the declared water route through the Startup Interface", async () => {
    ...
    expect(renderer.render).toHaveBeenCalledTimes(131);
    expect(renderer.readRenderTargetPixelsAsync).toHaveBeenCalledTimes(33);
    ...
    expect(readbackTargets[15]?.texture?.name).toBe(
      "Real Water unified foam sources",
    );
    expect(readbackTargets[32]?.texture?.name).toBe("Real Water final color");
  ...
  it("cleans partial water resources when progress presentation fails", async () => {
    ...
    expect(scene.children).toHaveLength(0);
    expect(renderer.compileAsync).toHaveBeenCalledTimes(2);
    expect(renderer.readRenderTargetPixelsAsync).toHaveBeenCalledTimes(16);
    expect(renderer.dispose).not.toHaveBeenCalled();
  });
  ...
  it("presents diagnostics outputs from the bound Core route without a second scene render", async () => {
    ...
    expect(all.outputs).toHaveLength(30);
