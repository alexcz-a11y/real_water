...
- Retain profile-sensitive Playwright Regression acceptance manifests as JSON under the gitignored `test-results/regression-acceptance/` directory, using unique names that survive parallel workers and retries. Attach the same file through Playwright `path` attachments. CI uploads only that directory after Playwright when files exist, and emits an explicit warning when profile-sensitive tests were unsupported or skipped; do not upload traces, authentication state, HAR files, or response bodies.
