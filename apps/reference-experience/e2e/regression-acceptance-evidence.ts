...
function readArtisticControls(value: unknown): ArtisticControls {
  if (!isRecord(value) || !hasExactKeys(value, ARTISTIC_CONTROL_KEYS)) {
    throw new TypeError(
      "Artistic controls must use the exact 15-key contract.",
    );
  }
...
function readQaPrewarmV11(
  ...
  if (
    ...
    throw new Error(
      "Regression acceptance requires the exact QA v11 30-name capture mapping.",
    );
  }
  if (QA_FRAME_PREWARM_MANIFEST.captures.length !== 30) {
    throw new Error("QA v11 capture contract must name exactly 30 outputs.");
  }
