...
test("gates TRAA warp residuals on a frozen-simulation fast pan", async ({
  ...
  const recipe = await page.evaluate(
    async ({
      ...
      let lastPrime: QaPresentationReceiptV5 | null = null;
      ...
      const captured: Array<{
        ...
        readonly prewarm: QaPresentationReceiptV5["prewarm"];
  ...
  const result = await page.evaluate(
    async ({ camera, seed }) => {
      ...
      const drive = async () => {
        ...
        await harness.setCamera(camera, { transition: "continuous" });
        const frames: Array<{
          readonly presentation: QaPresentationReceiptV5;
          readonly current: string;
          readonly final: string;
          readonly motion: string;
          readonly glint: string;
  ...
  const result = await page.evaluate(
    async ({ seed, setup, pan, cut }) => {
      ...
      async function primePresentedFrames(
        ...
        await qa.present();
        let presentation!: QaPresentationReceiptV5;
      ...
      async function capturePresentedBuffers(qa: QaHarnessV5) {
      ...
      async function readPresentedBuffers(qa: QaHarnessV5) {
        return {
...
function expectResetEquivalence(
  ...
  stable: QaPresentationReceiptV5,
